...
division.addEventListener('change', e => {
    e.preventDefault();
    if (division.value != '0') {
        var fetchData = fetch(`https://bdapis.herokuapp.com/api/v1.0/division/${division.value}`)
            .then(res => res.json())
            .then(res => res.data);
    ...
    district.addEventListener('change', e => {
        ...
        if (district.value != '0') {
            fetchData
                ...
                .then(res => res.upazilla.map(item => displayOptions(upazilla, item)))
            // .then(res => res.map(item => displayOptions(upazilla, item)))
        }
    });
});
...
document.getElementById('adress').addEventListener('submit', e=>{
    ...
    const address = `<div class="card">
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item text-capitalize">${division.value}</li>
                        <li class="list-group-item text-capitalize">${district.value}</li>
                        <li class="list-group-item text-capitalize">${upazilla.value}</li>
                    </ul>
                </div>`;
